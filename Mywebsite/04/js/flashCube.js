...
window.onload = function initWindow() {
	...
	thetaLoc = gl.getUniformLocation(program, "theta");//获取指定名称Uniform变量存储地址
	gl.uniform3fv(thetaLoc, theta);	//赋长度为3的数组
	...
	// task.b 实现旋转的立方体分别在X, Y, Z三条轴向上的平移
	...
	// task.c 实现旋转的立方体分别沿X, Y, Z三条轴向上的缩放
...
function render(){
	...
	if(trackballMove){
		...
		var cosa = Math.cos(angle/1.0);
		var sina = Math.sin(angle/1.0);
